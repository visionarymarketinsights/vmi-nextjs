  ...
  return (
    <div>
      <div className={`${segment !== 'request-sample' && 'md:sticky top-0'} pt-4 relative justify-between gap-2 bg-white border-b-2 md:flex px-4`}>
        <Link href={'/report/' + report?.url} className={`md:w-1/4 py-3 md:mb-0 mb-4 duration-200 text-sm flex justify-center rounded-b-none items-center rounded-sm cursor-pointer border-b-0 border-2 translate-y-[2px]  ${!segment ? 'font-semibold text-cyan-800 underline border-b-0 z-10 bg-white ' : ''}`}>Description</Link>
        <Link href={'/report/' + report?.url + '/toc'} className={`md:w-1/4 py-3 md:mb-0 mb-4 duration-200 text-sm rounded-b-none flex justify-center items-center border-b-0 border-2 z-10 rounded-sm cursor-pointer translate-y-[2px]  ${segment === 'toc' ? 'font-semibold text-cyan-800 underline border-b-0 z-10 bg-white' : ''}`}>Table Of Content</Link>
        <Link href={'/report/' + report?.url + '/methodology'} className={`md:w-1/4 py-3 md:mb-0 mb-4 duration-200 text-sm rounded-b-none flex justify-center items-center border-b-0 border-2 z-10 rounded-sm cursor-pointer translate-y-[2px]  ${segment === 'methodology' ? 'font-semibold text-cyan-800 underline border-b-0 z-10 bg-white' : ''}`}>Methodology</Link>
