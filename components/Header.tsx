        ...
        <div className="relative flex items-center justify-between md:mx-4">
          ...
          <div className="flex items-center justify-end w-full px-4">
            <div>
              ...
              <nav
                id="navbarCollapse"
                className={`absolute right-4 top-full w-full max-w-[250px] rounded-lg bg-white py-5 px-6 shadow lg:static lg:block lg:w-full lg:max-w-full lg:shadow-none z-20 ${
                  // !isMenuOpen && 
                  "hidden"
                  } `}
              >
                <div className="block lg:flex">
                  <div className="relative">
                    <Link className='flex py-2 text-base font-medium lg:ml-12 lg:inline-flex' 
                    // onClick={() => { setMenuOpen(!isMenuOpen); setCategoryOpen(false) }} 
                    href="/">Home</Link>
                  </div>
                  <div className="relative">
                  ...
                  <div className="relative">
                    <Link className='flex py-2 text-base font-medium lg:ml-12 lg:inline-flex' 
                    // onClick={() => { setMenuOpen(!isMenuOpen); setCategoryOpen(false) }} 
                    href="/all-press-release/agriculture">Press Release</Link>
                  </div>
                  <div className="relative">
                    <Link className='flex py-2 text-base font-medium lg:ml-12 lg:inline-flex' 
                    // onClick={() => { setMenuOpen(!isMenuOpen); setCategoryOpen(false) }} 
                    href="/offering">Offerings</Link>
                  </div>
                  < div className="relative" >
                    <Link className='flex py-2 text-base font-medium lg:ml-12 lg:inline-flex' 
                    // onClick={() => { setMenuOpen(!isMenuOpen); setCategoryOpen(false) }} 
