    ...
    = ({ url }: any) => {
        ...
        useEffect(() => {
            ...
            axios.get(`${apiUrl}/reports/category/category_count`).then(res => {
                let totalCount = 0;
                let tempCount = 0;
                let categoryList = res.data.data.map((r: any) => {
                        ...
                        <div className="pb-12 mx-auto max-w-8xl md:pb-10">
                            <div className='md:flex'>
                                ...
                                <div className="relative mt-12 md:w-3/4 md:ml-8 md:mt-0">
                                    ...
                                    {reportList.map((res: any, key) => {
                                        return (
                                            <Link href={`/report/${res.url}`} key={key}>
                                                <div className='mb-2 group' >
                                                    <div className='flex flex-col gap-2 p-4 border cursor-pointer group-hover:bg-slate-50'>
                                                        <div className="font-semibold group-hover:text-cyan-800 group-hover:underline">{res.title.split('').filter((res: any, i: number) => i < 160).join('')}... </div>
